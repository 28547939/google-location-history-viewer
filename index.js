    ...
    map.on('click', function (e) {
        map.forEachFeatureAtPixel(e.pixel, function (f) {
            ...
            if (typeof f.getId() == 'string') {
                 ...
                 * this specifically allows us to highlight en element in the table when its map feature is selected
                 */
...
/*
        ...
        */
        ...
        if (id.indexOf('placevisit') != -1) {
            ...
            // center the map over selected feature
            // this behavior may actually be annoying/not desired
                ...
                var new_line = function (x, id, is_waypoint) {
                    ...
                    var line = new Feature({
                        'geometry': new LineString([
                            // data needs to be shifted to the 'decimal form', then interpreted according to
                            // "EPSG:4326" and "EPSG:3857". if Google's format ever changes, some other kind
                            // of standard transformation may be necessary and it should be possible to figure
                            // it out by searching online.
                            transform([
                                x['start_lon'] / (10**7), 
                                x['start_lat'] / (10**7), 
                            ], 'EPSG:4326', 'EPSG:3857'), 
    ...
    for (var i = 0; i < data.length; i++) {
        if (data[i]['uniq_id'] == selected) {
            ...
            if (e.keyCode == 37 || e.keyCode == 38) {
                var j = (i == 0) ? (data.length - 1) : (i - 1); // wrap
    ...
    function (data, textStatus, jqXHR) {
        ...
        var processed = x.map(function (x) {
            ...
            if (x['type'] == 'placevisit') {
                ...
                // don't enumerate activitySegments, etc
                x['i'] = '';
            }
